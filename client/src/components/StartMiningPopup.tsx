...
export default function StartMiningPopup({ uid, onClose }: StartMiningPopupProps) {
  ...
  const handleNormalMining = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: err.message || "Mining start failed",
        variant: "destructive",
      });
    }
  };
  ...
  const handleAdMining = async () => {
    try {
      ...
      toast({
        title: "Ad Error",
        description: err.message || "Ad failed",
        variant: "destructive",
      });
    }
  };
  ...
  const startMiningAfterAd = async () => {
    try {
      ...
      if (result.status !== "success") {
        throw new Error(result.message || "Failed to start mining after ad");
      }
      ...
      toast({
        ...
        description: "24h mining activated after ad!",
      });
      onClose();
    } catch (err: any) {
      setWaitingAd(false);
