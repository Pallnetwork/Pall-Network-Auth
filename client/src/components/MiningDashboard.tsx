...
export default function MiningDashboard() {
  ...
  const handleDailyReward = async () => {
    ...
    if (window.AndroidBridge?.startDailyRewardedAd) {
      ...
      if (res.status==="success") { setUiBalance(prev=>prev+0.1); setClaimedCount(prev=>prev+1); toast({title:"🎉 Reward Received", description:"+0.1 Pall added"});}
      else toast({ title:"Daily Reward", description:res.message||"Already claimed", variant:"destructive"});
      setDailyWaiting(false);
      waitingForAdRef.current = false;
